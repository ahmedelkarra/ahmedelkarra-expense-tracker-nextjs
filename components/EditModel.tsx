...
export default function EditModel({ item }: { item: IExpenses }) {
    const [valueInputs, setValueInputs] = useState<IExpenses>({ _id: '', title: '', price: 0, status: '' })
    ...
    const handelAdd = () => {
        if (valueInputs.price < 0) {
            ...
            AxiosExpenses.put('/expenses', valueInputs, { headers: { Authorization: localStorage.getItem('token') } })
                .then((e) => {
                    ...
                    setTimeout(() => {
                        ...
                        setHandelError('')
                        setHandelSuccess('')
                        setValueInputs({ _id: '', title: '', price: 0, status: '' })
                    }, 2000)
                })
                .catch((err) => {
    ...
    const handelClick = () => {
        setShowModal(true)
        setValueInputs({ _id: item._id, price: item.price, status: item.status, title: item.title })
    }
    return (
        <>
            <button
                className='p-2 text-[12px] sm:text-[15px] text-white rounded-md bg-yellow-500 col-span-12 lg:col-span-6'
                type="button"
                onClick={handelClick}
            >
                Edit
                        ...
                        <div className="justify-center items-center flex relative w-auto my-6 mx-auto max-w-3xl">
                            ...
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col bg-white outline-none focus:outline-none w-[80dvw] lg:w-[70dvw]">
                                ...
                                <div className="grid grid-cols-12 gap-2 relative p-6">
                                    <input type="text" className="col-span-12 lg:col-span-6 border p-3 rounded-md" placeholder="Enter Title" maxLength={50} onChange={(e) => setValueInputs({ ...valueInputs, title: e.target.value.toLocaleLowerCase().trim() })} required value={valueInputs?.title} />
                                    <input type="number" className="col-span-12 lg:col-span-6 border p-3 rounded-md" placeholder="Enter Price" min={0} maxLength={10} onChange={(e) => setValueInputs({ ...valueInputs, price: Number(e.target.value) })} required value={valueInputs?.price} />


                                    <div className="col-span-6 flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700">
                                        <input checked={valueInputs.status === 'debit'} onChange={(e) => setValueInputs({ ...valueInputs, status: e.target.value.toLocaleLowerCase().trim() })} id="bordered-radio-1" type="radio" value="debit" name="bordered-radio" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                    ...
                                    <div className="col-span-6 flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700">
                                        <input checked={valueInputs.status === 'credit'} onChange={(e) => setValueInputs({ ...valueInputs, status: e.target.value.toLocaleLowerCase().trim() })} id="bordered-radio-2" type="radio" value="credit" name="bordered-radio" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
